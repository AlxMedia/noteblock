...
/* ------------------------------------ */
if ( ! function_exists( 'noteblock_hex2rgb' ) ) {

	function noteblock_hex2rgb( $hex, $array=false ) {
		$hex = str_replace("#", "", $hex);
...
/*  Google fonts
/* ------------------------------------ */
if ( ! function_exists( 'noteblock_enqueue_google_fonts' ) ) {

	function noteblock_enqueue_google_fonts () {
...
add_action( 'wp_enqueue_scripts', 'noteblock_enqueue_google_fonts' ); 	


/*  Dynamic css output
/* ------------------------------------ */
if ( ! function_exists( 'noteblock_dynamic_css' ) ) {

	function noteblock_dynamic_css() {
		if ( get_theme_mod('dynamic-styles', 'on') == 'on' ) {
		
			// rgb values
			$color_1 = get_theme_mod('color-1');
			$color_1_rgb = noteblock_hex2rgb($color_1);
			...
			// gradient left
			if ( get_theme_mod('gradient-left','#00c498') != '#00c498' ) {
				$styles .= '
...
#wrap-nav-header,
#wrap-nav-mobile,
.entry-list:hover .entry-title { background-image: linear-gradient(to left,'.esc_attr( get_theme_mod('gradient-right') ).','.esc_attr( get_theme_mod('gradient-left') ).'); }	
				'."\n";
			}
			// gradient right
			if ( get_theme_mod('gradient-right','#1c6da1') != '#1c6da1' ) {
				$styles .= '
#wrapper-inner:before,
#header .social-links .social-tooltip:hover,
#profile-image,
...
.entry-list:hover .entry-title { background-image: linear-gradient(to left,'.esc_attr( get_theme_mod('gradient-right') ).','.esc_attr( get_theme_mod('gradient-left') ).'); }	
			...
			// sidebar color
			if ( get_theme_mod('color-sidebar','') != '' ) {
				$styles .= '
#wrapper { background-color: '.esc_attr( get_theme_mod('color-sidebar') ).'; }
				'."\n";
			}
			// background color
			if ( get_theme_mod('color-background','#100e17') != '#100e17' ) {
				$styles .= '
...
.entry a { color: '.esc_attr( get_theme_mod('color-link') ).'; }
			...
			wp_add_inline_style( 'noteblock-style', $styles );	
		}
	}
	
}
add_action( 'wp_enqueue_scripts', 'noteblock_dynamic_css' );
